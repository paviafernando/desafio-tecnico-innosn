...
- Las notificaciones viven solo en memoria (no se persisten en `localStorage` ni en base de datos): al refrescar la página se pierden. Se consideró suficiente para el alcance del desafío; si hiciera falta persistencia entre sesiones, el camino natural sería una tabla `notificaciones` server-side, fuera de alcance ahora.
